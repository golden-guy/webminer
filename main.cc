    ...
    while (!g_shutdown) {
        ...
        while (true) {
            ...
            if (apparent_difficulty < current_difficulty) {
                ...
                orphan_log << soln.preimage << ' ' << absl::BytesToHexString(absl::string_view((const char*)soln.hash.begin(), 32)) << ' ' << soln.webcash << " difficulty=" << apparent_difficulty << std::endl;
                orphan_log.flush();
                continue;
            }
...
void mining_thread_func(int id)
{
    ...
    while (!done) {
        ...
        unsigned char hashes[batch_size*32] = {0};
        ...
        for (int i = 0; i < 1000; ++i) {
            for (int j = 0; j < 1000; j += batch_size) {
                ...
                for (int k = 0; k < batch_size; ++k) {
                    ...
                    SHA256_Final(hashes + k*32, &sha256_final);
                }
#else
